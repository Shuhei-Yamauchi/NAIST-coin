        ...
        bool sign) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
   ...
   //std::cout << coin_control.m_max_depth << std::endl;
...
if (coin_control.CoinType == "asendtoaddress" ){
   std::cout << "/* This is Account Transaction */" ;
   //std::cout << coin_control.address_to ;

//std::generate(txNew.vin.begin(), txNew.vin.end(), std::rand);

    txNew.address_to = coin_control.address_to;
    txNew.address_from = coin_control.address_from;
    std::cout << ".address_to   : " + txNew.address_to + "\n";
    std::cout << ".address_from : " + txNew.address_from+ "\n";
    txNew.amount = coin_control.amounts;
    //txNew.nonce = rand();
    //txNew.nVersion = 5;
    txNew.vin.clear();
    //txNew.vin[0].
    //txNew.vin[0].dummy_CTxIn = rand();
    txNew.vout.clear();
    txNew.accountNonce = rand();
//    txNew.nLockTime = rand();

    //txNew.vout.resize(0);
    //    txNew.vout[0].nValue = rand();
    //txNew.vout[0].dummy_CTxOut = rand();
    txNew.transactionType = "account";
    //txNew.address_from = "abcdefg";
    //txNew.accountNonce = nonceA++
//        std::cout << "transaction-type-is " + txNew.transactionType + "\n";
//    std::cout << "55555" + txNew.address_to + "\n";
//    std::cout << "66666" + txNew.address_from + "\n";
      std::cout << "txNew.amount is :" + txNew.amount + "\n";
            //std::cout << "nonce = " + std::to_string(txNew.nonce) + "\n";
            std::cout << "nlocktime = " + std::to_string(txNew.nLockTime) + "\n";
//const_cast<std::string&>.CoinType) = "" ;
...
std::string pubkeystring = HexStr(pubkey);
std::cout << pubkeystring << std::endl;

CPrivKey priv = key.GetPrivKey();
std::string privstring = HexStr(priv);
std::cout << privstring << std::endl;

auto testest = key.VerifyPubKey(pubkey);
*/
//#include "hash.h"
//key.Load(priv,pubkey,true);
#define 	NID_secp192k1   711
 ...
 if (!EC_KEY_generate_key(eckey))
	...
	char *accountMessage = {"Sample Message"};
    txNew.accountMessage = accountMessage;
    txNew.testMessage = accountMessage;
    txNew.testMessage  = "donaldo";
	//unsigned char digest[SHA256_DIGEST_LENGTH];
	unsigned char digest[SHA256_DIGEST_LENGTH];
	SHA256_CTX sha_ctx;
	SHA256_Init(&sha_ctx); // コンテキストを初期化
	SHA256_Update(&sha_ctx, accountMessage, sizeof(accountMessage)); // message を入力にする
	SHA256_Final(digest, &sha_ctx); // digest に出力

     sig = ECDSA_do_sign(digest,32,eckey);

        std::string strMsg = "Very secret message";
...
// //wallet.ve()
// key2.Load(wa)
// key2.Sign(hashMsg, sign1);

 if (sig == NULL){
     std::cout << "sig is NULL\n\n\n";
 }
...
ret = ECDSA_do_verify(digest, 32, sig, ecpub);

//CKey test01 = dynamic_cast<CKey*>(eckey);
//CPubKey test02 = static_cast<CPubKey>(ecpub);

    // FillableSigningProvider keystore;
    // keystore.AddKeyPubKey(static_cast<CKey>(eckey),static_cast<CPubKey>(ecpub));


//memcpy(txNew.digest,digest,sizeof(digest));
//txNew.digest = digest;
txNew.sig = sig;
txNew.pubkey = ecpub;
...
//PartiallySignedTransaction(txNew);
    ...
    //This is Transaction making
    //02-16 Shuhei Yamauchi
    if (coin_control.m_max_depth == 8888888)
    {
    std::cout << "/* This is dummy Transaction */" ;
...
/*
std::cout << txNew.vin.empty() << std::endl;
std::cout << txNew.vout.empty() << std::endl;
//CTxIn vinDummy;
//CTxOut voutDummy;
*/

txNew.vout[0].dummy_CTxOut = 1;
std::cout<<"testMessage is ";
std::cout<<txNew.testMessage;
std::cout<<"\n";
//std::cout << "test//Message is " + std::tmpstring(txNew.testMessage) + "\n"  + std::endl;

//std::cout << "dummy^CTxOUT is " + std::to_string(txNew.vout[0].dummy_CTxOut);

// //std::cout << txNew.vin.empty() << std::endl;
// //std::cout << txNew.vout.empty() << std::endl;
// std::cout << "transactionTyp is " + txNew.transactionType + "\n";
// std::cout << "address_to is " + txNew.address_to + "\n";
// std::cout << "address_from is " + txNew.address_from + "\n";
// std::cout << "START is OK\n\n\n";
//     // Return the constructed transaction data.
     tx = MakeTransactionRef(std::move(txNew));
// std::cout << "END is OK\n\n\n";
// std::cout << "address_to is " + tx->address_to + "\n";
// std::cout << "address_from is " + tx->address_from + "\n";
// std::cout << "transactionTyp is " + tx->transactionType + "\n";
// //std::cout << "testMessage is " + txNew.testMessage + "\n";
// std::cout << "dummy^CTxOUT is " + std::to_string(tx->vout[0].dummy_CTxOut);
...
std::cout << "transactionTyp is " + tx.get()->transactionType + "\n";

 //std::shared_ptr<std::string> tx.reset(new std::string("3"));
 //std::shared_ptr<std::string> t (new std::string("foo"));
