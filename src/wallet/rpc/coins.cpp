...
RPCHelpMan agetbalance()
{
    return RPCHelpMan{"agetbalance",
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    //bool avoid_reuse = GetAvoidReuseFlag(*pwallet, request.params[3]);
        ...
        for (const auto& entry : pwallet->mapWallet) {
            ...
            const CWalletTx& wtx = entry.second;
                ...
                std::cout << "[balance] tx type is = "  + wtx.tx->vout[0].transactionType + "\n";
            std::cout <<"[balance] address-to "+ wtx.tx->vout[0].vout_address_to+ "\n";
    ...
    std::cout << wtx.tx->amount << std::endl;
    //std::cout << bool(mine2 & ISMINE_SPENDABLE) << std::endl;     
    std::cout << "[agetbalance] address-to is : "  + wtx.tx->address_to+ "\n";
    std::cout << "[agetbalance] address-from is : "  + wtx.tx->address_from+ "\n";         
...
std::cout <<"[balance]tx-amount "+ std::to_string(tx_amount) + "\n";
    ...
    //const auto bal = GetBalance(*pwallet, min_depth, avoid_reuse);
        //return strprintf("a");
//CAmount dona = 52 ;
int64_t quotient = tx_amount * COIN;
    // return UniValue(UniValue::VNUM,
    //         strprintf("%s%d.%08d", dona < 0 ? "-" : "", quotient, remainder));
    return ValueFromAmount(quotient + (include_watchonly ? 0 : 0));
},
    };
}
